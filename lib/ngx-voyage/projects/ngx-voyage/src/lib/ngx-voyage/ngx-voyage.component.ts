...
export class NgxVoyageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const hasPermissionManagements = this.permissionsManagement["listeners"]?.length > 0;
    const hasModifyFile = this.modifyFile["listeners"]?.length > 0;
    const hasDeleteFile = this.deleteFile["listeners"]?.length > 0;
    this.store.showOpenFile.set(hasOpenFileOutput);
    this.store.showPreviewFile.set(hasPreviewFileOutput);
    this.store.showPermissionManager.set(hasPermissionManagements);
    this.store.showModifyFile.set(hasModifyFile);
    this.store.showDeleteFile.set(hasDeleteFile);
